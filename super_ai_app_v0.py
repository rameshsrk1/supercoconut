...
class GemmaChatLLM(LLM):
    model_id: str = "gpt-4o-mini"  # Use the desired model
    temperature: float = 0.7
    max_tokens: int = 1024
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            raise ImportError("The `openai` package is required for this functionality.")

        # Simple test call
        resp = client.Completion.create(
            model=self.model_id,
            prompt="Hello, explain PCIe in simple words",
            max_tokens=50
        )
