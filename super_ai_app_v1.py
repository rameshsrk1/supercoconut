...
with st.sidebar:
    ...
    st.markdown(
        "<hr><div style='text-align:center;font-size:0.95rem; color:#8aa;'>Made with ❤️ using Streamlit</div>",
...
for idx, (user, ai) in enumerate(st.session_state.chat_history):
    st.markdown(
        f"<div class='bubble-user'>{user}<div class='chat-timestamp'>You</div></div>", unsafe_allow_html=True
    )
    st.markdown(
        f"<div class='bubble-ai'>{ai}<div class='chat-timestamp'>AI</div></div>", unsafe_allow_html=True
    )
